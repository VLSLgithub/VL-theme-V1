...
class Foundationpress_Comments extends Walker_Comment {
	...
	function __construct() { ?>
		...
		<h3><?php comments_number( __( 'No Responses to', 'vibrant-life-theme' ), __( 'One Response to', 'vibrant-life-theme' ), __( '% Responses to', 'vibrant-life-theme' ) ); ?> &#8220;<?php the_title(); ?>&#8221;</h3>
	...
	function start_el( &$output, $comment, $depth = 0, $args = array(), $id = 0 ) {
		...
		<header class="comment-author">
			...
			<?php
				...
				printf( __(
					'<cite class="fn">%s</cite>', 'vibrant-life-theme' ),
			...
			<time datetime="<?php echo comment_date( 'c' ) ?>"><a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ) ?>"><?php printf( get_comment_date(), get_comment_time() ) ?></a></time>
				...
				<section id="comment-content-<?php comment_ID(); ?>" class="comment">
					<?php if ( ! $comment->comment_approved ) : ?>
							<div class="notice">
					<p class="bottom"><?php _e( 'Your comment is awaiting moderation.', 'vibrant-life-theme' ); ?></p>
				</div>
